...
function M.blend(foreground, alpha, background)
  if background == "NONE" or foreground == "NONE" then
    return "NONE"
  end
...
function M.invert(color)
  ...
  elseif type(color) == "string" then
    local hsluv = require("arrowlake.hsluv")
    if color ~= "NONE" then
...
---@param groups arrowlake.Highlights
...
function M.cache.file(key)
  return vim.fn.stdpath("cache") .. "/arrowlake-" .. key .. ".json"
end

---@param key string
function M.cache.read(key)
  ---@type boolean, arrowlake.Cache
